...
from httpx import HTTPError, RequestError, InvalidURL
...
class Auth0Manager:
    def __init__(self,
        ...
        if not re.search(r'https?://', fqdn):
            self.api_identifier = f"http{'s' if not fqdn.startswith('localhost') else ''}://{fqdn}"
        self.httpx = httpx.Client()
        self.token = self.get_access_token()
        self.httpx.headers.update({"Authorization": f"Bearer {self.token.access_token}",
    ...
    def request_access_token(self) -> Token:
        ...
        for c in range(4):
            ...
            try:
                ...
                token = Token(res['access_token'], token_expiry= time() + int(res['expires_in']))
                return token
        else:
    ...
    def update_client(self, client_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        response = self.httpx.patch(f"{self.base_url}/api/v2/clients/{client_id}", json=data)
        return response.json()
    
    @property
    def client_grants(self):
        response = self.httpx.get(f"{self.base_url}/api/v2/client-grants", params={
            "client_id": self.mtm_client_id,
            "audience": self.audience,
        })
        return response.json()  
    ...
    def update_client_grant(self, client_grant_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        response = self.httpx.patch(f"{self.base_url}/api/v2/client-grants/{client_grant_id}", json=data)
        return response.json()
    
    def delete_client_grant(self, client_grant_id: str):
        response = self.httpx.delete(f"{self.base_url}/api/v2/client-grants/{client_grant_id}")
        return response.json()
    ...
    def get_resource_server(self, server_id: str) -> Dict[str, Any]:
        response = self.httpx.get(f"{self.base_url}/api/v2/resource-servers/{server_id}")
        return response.json()
    
    def create_resource_server(self, data: Dict[str, Any]) -> Dict[str, Any]:
        response = self.httpx.post(f"{self.base_url}/api/v2/resource-servers", json=data)
        return response.json()
    
    def update_resource_server(self, server_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
        response = self.httpx.patch(f"{self.base_url}/api/v2/resource-servers/{server_id}", json=data)
        return response.json()
    
    def initialize_global_client_grants(self):
        required_scopes = ["read:client_credentials", "read:clients", "update:clients", "create:clients", "delete:clients",
        ...
        if 'error' in client_grants:
            exit("Seems like you haven't given the mandatory scopes to the machine-to-machine [MTM] application, \
        ...
        for grant in self.client_grants:
            if grant['client_id'] == self.mtm_client_id:
                ...
                if not set(grant["scope"]) == set(required_scopes):
                    ...
                    resp = self.update_client_grant(grant['id'], {"scope": required_scopes})
                    if 'error' in resp:
                        print(resp)
    ...
    def initialize_api(self):
        ...
        for api in resource_servers:
            if api.get('name') == "Dester" and api.get('identifier') == self.api_identifier:
                print("Found the api")
                if not api.get('signing_alg') == "RS256":
                    print("Signing algorithm is incorrect, updating..")
                    api = self.update_resource_server(api['id'], {
                        'signing_alg': "RS256"
                    })
                if not set([obj.get('value') for obj in api.get("scopes", [])]) == set(required_scopes):
                    print("Insufficient scopes, updating..")
                    api = self.update_resource_server(api['id'], {
                        "scopes": [
                            {"value": scope,
                            "description": f"Auto-added scope [{scope}]"} for scope in required_scopes
                        ]
                    })
                ...
                if not api.get("skip_consent_for_verifiable_first_party_clients"):
                    print("Consent for verifiable first party clients is disabled, updating..")
                    api = self.update_resource_server(api['id'], {
        ...
        for client in self.clients:
            if client.get("app_type") == "non_interactive" and client.get("name") == "Dester [API]":
                ...
                if not client.get("token_endpoint_auth_method") == "client_secret_post":
                    ...
                    client = self.update_client(mtm_client_id, {"token_endpoint_auth_method": "client_secret_post"})
                if not client.get("oidc_conformant"):
                    print("Updating oidc conformant..")
                    client = self.update_client(mtm_client_id, {"oidc_conformant": True})
                if not client.get("grant_types") == ["client_credentials"]:
                    print("Updating grant types..")
                    client = self.update_client(mtm_client_id, {"grant_types": ["client_credentials"]})
                break
        else:
            print(f"No machine-to-machine APP found for API \'{api.get('name')}\', creating one..")
            ...
            self.update_client_grant(client.get('client_id'),{
                'audience': self.audience,
        ...
        for client in self.clients:
            if client.get("app_type") == "spa":
                ...
                if client.get("callbacks") != [self.api_identifier]:
                    ...
                    client = self.update_client(spa_client_id, {"callbacks": [self.api_identifier]})
                if client.get("web_origins") != [self.api_identifier]:
                    print("Updating web origins..")
                    client = self.update_client(spa_client_id, {"web_origins": [self.api_identifier]})
                if client.get("allowed_logout_urls") != [self.api_identifier]:
                    print("Updating allowed logout urls..")
                    client = self.update_client(spa_client_id, {"allowed_logout_urls": [self.api_identifier]})
                break
        else:
...
if __name__ == "__main__":
    ...
    if not global_client_secret:
        global_client_secret = '-fpIGmiiigiUx3kKl6WZhXZgr6lMLQVSdz46NEiqrwjvSdu4qE4Xx-la52grhJHq'
    api_identifier = input("Enter your api url which will be used for configuring callbacks, web origins, etc:> ")
    mg = Auth0Manager(
        domain=domain,
