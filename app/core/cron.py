...
def fetch_metadata():
    ...
    for key, category in rclone.items():
        ...
        if category_metadata["type"] == "series":
            series_metadata.extend(generate_series_metadata(
                tmdb, rclone[key].fetch_series(), key
            ))
        else:
            movies_metadata.extend(generate_movie_metadata(
                tmdb, rclone[key].fetch_movies(), key
            ))
    ...
    if len(series_metadata) > 0:
        mongo.movies_col.bulk_write(series_metadata)
