...
class Movie:
    __slots__ = ['id', 'file_name', 'path', 'parent', 'modified_time', 'number_of_files', 'rclone_index', 'tmdb_id', 'title', 'original_title', 'status', 'popularity', 'revenue', 'rating',
                 'release_date', 'year', 'tagline', 'description', 'cast', 'crew', 'collection', 'genres', 'external_ids', 'logo_path', 'homepage', 'thumbnail_path', 'backdrop_path', 'poster_path', 'videos', 'reviews']

    def __dict__(self):
        return {"id": self.id, "file_name": self.file_name, "path": self.path, "parent": self.parent, "modified_time": self.modified_time, "number_of_files": self.number_of_files,
                "rclone_index": self.rclone_index, "tmdb_id": self.tmdb_id, "title": self.title, "original_title": self.original_title,
                "status": self.status, "popularity": self.popularity, "revenue": self.revenue, "rating": self.rating, "release_date": self.release_date, "year": self.year,
                "tagline": self.tagline, "description": self.description, "cast": self.cast, "crew": self.crew, "collection": self.collection, "genres": self.genres,
                "external_ids": self.external_ids, "logo_path": self.logo_path, "homepage": self.homepage, "thumbnail_path": self.thumbnail_path,
                "backdrop_path": self.backdrop_path, "poster_path": self.poster_path, "videos": self.videos, "reviews": self.reviews
                }
