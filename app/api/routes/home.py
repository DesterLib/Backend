...
def home() -> Dict[str, str]:
    ...
    with Cache("cache", timeout=0) as cache:
        ...
        if not data:
            ...
            cache.set("top_rated_movies_data", top_rated_movies_data)
            cache.set("newly_added_movies_data", newly_added_movies_data)
            cache.set("newly_added_episodes_data", newly_added_episodes_data)
        else:
            categories_data = cache.get("categories_data")
            carousel_data = cache.get("carousel_data")
            top_rated_movies_data = cache.get("top_rated_movies_data")
            top_rated_series_data = cache.get("top_rated_series_data")
            newly_added_episodes_data = cache.get("newly_added_episodes_data")
            newly_added_movies_data = cache.get("newly_added_movies_data")
